...
| **Generic scored-item search helper** | 5th preference-scored item search function added (currently 1: `findPreferredItemByType`; steps 11c–11d will add scored vessel and plantable search). Evaluate whether a shared `findBestItem(items, filterFn, scoreFn)` reduces meaningful duplication without adding indirection overhead. |
