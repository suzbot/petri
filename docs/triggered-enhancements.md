...
| **Generic scored-item search helper** | 5th preference-scored item search function added (currently 2: `findPreferredItemByType`, `findPreferredPlantableOnGround`; `FindAvailableVessel` and `FindVesselContaining` also gained inline scoring in Step 11d). Evaluate whether a shared `findBestItem(items, filterFn, scoreFn)` reduces meaningful duplication without adding indirection overhead. |
| **findCraftIntent preference scoring validation** | Competing craft recipes added for the same activity (e.g., clay pot vessel alongside gourd vessel). Verify `findCraftIntent` scores all feasible recipes and selects the one matching character preferences, not just first-feasible. Infrastructure built in Step 11c — needs real test coverage when there's a second recipe to compete. |
| **Multi-material or primary-material marking on recipes** | Competing craft recipes added (e.g., clay pot vs gourd vessel); Material field on Item (DD-61) needs to distinguish identity-defining material from generic components for multi-input recipes. Current approach: single Material field, primary input derived by convention. Expand when: recipe with 2+ identity-relevant inputs exists; or material preference scoring produces wrong results because the wrong input is treated as primary. Options: PrimaryInput flag on RecipeInput, multiple Material fields, or derive from output Kind string matching. |
| **2nd-bug-rule computational enforcement** | Simplified enforcement (CLAUDE.md feedforward rule + visible "Fix #N" recap) gets skipped 3+ times — explicit fix-naming fails to fire despite the rule being loaded. Evaluate computational controls (e.g., PostToolUse hook tracking file-write operations during testing phases) rather than memory counter infrastructure. |
| **Monitor for test observation in `/fix-bug`** | Test-output reporting becomes repeated friction during `/fix-bug` flows — 3+ instances where verbose output or multi-failure sequences cause rework because Claude doesn't observe raw output directly. Wire Monitor tool into the test-observation step of `/fix-bug` and `/test-world`. |
