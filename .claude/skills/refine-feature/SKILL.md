...
- For new entity fields or existing fields newly populated on an item type: trace all code paths that read or write the parent struct, including ToSaveState/FromSaveState. Old saves will lack the field — spec must include migration in `itemFromSave()`.
- For display text changes: grep all rendering paths that produce the text the user sees — not just the method being modified. UI text often has parallel paths (e.g., `DisplayName()` for existing orders vs `activity.Name` in the order creation submenu). The spec must name the actual rendering path, not assume a helper method covers it.
