...
|------|----------|-------------|
| `README.md` | **Players** | Primarily Latest Updates section. Other sections only if absolutely needed. Player-visible changes only. No implementation details, no specific counts or enumerations (not "seven variants" — just "multiple variants" or omit). One line per feature, plain language. Brief summary, doesn't need to cover details that will be in mechanics doc. Only if a major new workflow, make a brief update to 'How it works'. |
